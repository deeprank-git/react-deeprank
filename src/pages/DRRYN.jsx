...
import drrynImg from "../assets/yogalife.png";
...
const performanceData = [
  { m: "Jan", organic: 400, leads: 4, conversions: 1 },
  { m: "Feb", organic: 700, leads: 9, conversions: 3 },
  { m: "Mar", organic: 1100, leads: 16, conversions: 6 },
  { m: "Apr", organic: 1600, leads: 28, conversions: 12 },
  { m: "May", organic: 2300, leads: 42, conversions: 24 },
  { m: "Jun", organic: 3200, leads: 68, conversions: 40 },
];

const TABS = ["Overview", "About The Client", "Our Strategy", "The Results", "Testimonial"];

const relatedCases = [
  { img: drrynImg, tag: "Retail", title: "DRRYN", stat: "+420%", label: "Sales", desc: "Ecommerce growth for modern retailers." },
    ...
    <div className="bg-white text-slate-900">
      <div className="max-w-7xl mx-auto px-5 lg:px-8 pt-6 pb-2">
        <nav className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <HiChevronRight className="w-3.5 h-3.5" />
          <span className="text-gray-900 font-semibold">DRRYN</span>
        </nav>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-5 lg:px-8 py-10 lg:py-14">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="text-4xl lg:text-5xl font-black leading-[1.08] text-gray-900 mb-3">DRRYN</h1>
            <h2 className="text-3xl lg:text-4xl font-black text-blue-700 leading-tight mb-5">420% Increase In Online Sales</h2>
            <p className="text-gray-500 text-base leading-relaxed mb-6 max-w-lg">DRRYN partnered with DeepRank to scale ecommerce sales across marketplaces and direct channels.</p>
            <div className="flex flex-wrap gap-2 mb-8">{["Ecommerce","SEO","PPC","UX"].map(t=>(<span key={t} className="text-xs font-semibold text-blue-700 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-full">{t}</span>))}</div>
          ...
          <div className="relative"><div className="rounded-3xl overflow-hidden shadow-2xl"><img src={drryn} alt="DRRYN" className="w-full h-80 lg:h-96 object-cover"/></div></div>
        </div>
      </section>

      <div className="bg-gray-50 border-y border-gray-100"><div className="max-w-7xl mx-auto px-5 lg:px-8 py-4"><div className="flex flex-wrap gap-6 lg:gap-12">{[{label:"Industry",value:"Retail"},{label:"Location",value:"Global"},{label:"Duration",value:"8 Months"},{label:"Services Used",value:"Ecommerce + SEO + PPC"}].map(i=>(<div key={i.label} className="flex items-center gap-3"><div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{i.label}</p><p className="text-sm font-bold text-gray-900 mt-0.5">{i.value}</p></div></div>))}</div></div></div>
      ...
      <div className="max-w-7xl mx-auto px-5 lg:px-8 py-12 lg:py-16 space-y-16">
        <div className="grid lg:grid-cols-3 gap-8"><div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3">About The Client</p><h3 className="text-xl font-black text-gray-900 mb-4">DRRYN</h3><p className="text-sm text-gray-600 leading-relaxed">DRRYN is a modern retail brand focused on direct-to-consumer growth and marketplace performance.</p></div><div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3">The Challenge</p><h3 className="text-xl font-black text-gray-900 mb-4">What We Had to Solve</h3><ul className="flex flex-col gap-2.5">{["Low AOV","Marketplace dependency","High CAC","Fragmented analytics"].map(i=>(<li key={i} className="flex items-start gap-2 text-sm text-gray-600"><HiXCircle className="w-4 h-4 text-red-400 shrink-0 mt-0.5"/>{i}</li>))}</ul></div><div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3">Before Working With Us</p><h3 className="text-xl font-black text-gray-900 mb-4">Starting Point</h3><div className="grid grid-cols-2 gap-3">{[{v:"400",l:"Monthly Traffic"},{v:"20",l:"Orders / Month"},{v:"£35",l:"AOV"},{v:"0.8%",l:"Conversion Rate"}].map(s=>(<div key={s.l} className="bg-red-50 border border-red-100 rounded-2xl p-4 text-center"><p className="text-2xl font-black text-red-600">{s.v}</p><p className="text-[10px] text-gray-500 font-medium mt-1">{s.l}</p></div>))}</div></div></div>

        <div><div className="mb-8"><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-2">Our Strategy</p><h3 className="text-3xl font-black text-gray-900">Omnichannel growth for direct-to-consumer brands.</h3></div><div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">{[{n:"01",title:"Marketplace SEO",desc:"Optimize listings & traffic"},{n:"02",title:"Site UX",desc:"Faster checkout"},{n:"03",title:"Paid Growth",desc:"ROAS-focused media"},{n:"04",title:"Retention",desc:"Email + SMS flows"},{n:"05",title:"Analytics",desc:"Unified reporting"}].map(step=>(<div key={step.n} className="bg-gray-50 rounded-2xl p-5 border border-gray-100"><span className="text-4xl font-black text-blue-100">{step.n}</span><h4 className="text-sm font-black text-gray-900 mt-2 mb-2">{step.title}</h4><p className="text-xs text-gray-500 leading-relaxed">{step.desc}</p></div>))}</div></div>

        <div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-6">Services Used</p><div className="flex flex-wrap gap-3">{"Ecommerce,SEO,PPC,Retention,Analytics".split(',').map(s=>(<span key={s} className="rounded-full border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm">{s}</span>))}</div></div>

        <div className="grid lg:grid-cols-2 gap-12 items-start"><div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3">What We Did</p><h3 className="text-3xl font-black text-gray-900 mb-6">End-to-end ecommerce execution.</h3><div className="grid sm:grid-cols-2 gap-x-8 gap-y-2.5">{"Listing optimisation,Checkout rebuild,Paid campaigns,Retention automation,Marketplace ops,Analytics,Creative,Support".split(',').map(item=>(<div key={item} className="flex items-center gap-2 text-sm text-gray-700"><HiCheckCircle className="w-4 h-4 text-blue-600 shrink-0"/>{item}</div>))}</div></div><div className="bg-blue-900 rounded-3xl p-8 text-white"><p className="text-5xl text-blue-400 font-black leading-none mb-4">"</p><p className="text-base leading-relaxed text-blue-100 mb-6">"DRRYN scaled faster than expected across channels with clearer unit economics."</p><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-blue-700 flex items-center justify-center text-white font-black text-sm">DR</div><div><p className="text-sm font-bold">DRRYN CEO</p><p className="text-xs text-blue-300">Founder</p></div></div></div></div>

        <div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-2">The Results</p><h3 className="text-3xl font-black text-gray-900 mb-10">Orders and revenue climbed rapidly.</h3>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">{[{label:"Orders",before:"20",after:"104",delta:"+420%",color:"#3B82F6",grad:"og",data:trafficData,positive:true},{label:"Traffic",before:"400",after:"3.2K",delta:"+700%",color:"#8B5CF6",grad:"le",data:trafficData,positive:true},{label:"AOV",before:"£35",after:"£52",delta:"+48%",color:"#10B981",grad:"cp",data:leadsData,positive:true},{label:"Conversion",before:"0.8%",after:"3.2%",delta:"+300%",color:"#F59E0B",grad:"ic",data:leadsData,positive:true}].map(m=>(<div key={m.label} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-3">{m.label}</p><div className="flex items-end justify-between mb-1"><p className="text-3xl font-black text-gray-900">{m.after}</p><span className={`text-xs font-bold px-2 py-0.5 rounded-lg ${m.positive?"bg-green-50 text-green-700":"bg-red-50 text-red-700"}`}>{m.delta}</span></div><p className="text-[10px] text-gray-400 mb-3">was {m.before}</p><MetricChart data={m.data} color={m.color} gradient={m.grad} /></div>))}</div>

          <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm"><div className="flex items-center justify-between mb-1"><p className="text-sm font-black text-gray-900">Performance Over Time</p><span className="text-[10px] text-gray-400 bg-gray-50 px-2.5 py-1 rounded-lg">Jan – Jun 2025</span></div><p className="text-[11px] text-gray-400 mb-4">Monthly growth across channels</p><div className="h-[240px]"><ResponsiveContainer width="100%" height="100%"><LineChart data={performanceData} margin={{top:4,right:8,left:-20,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="#F3F4F6"/><XAxis dataKey="m" tick={{fontSize:10,fill:"#9CA3AF"}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:10,fill:"#9CA3AF"}} axisLine={false} tickLine={false}/><Tooltip contentStyle={{fontSize:11,borderRadius:8,border:"1px solid #E5E7EB"}}/><Line type="monotone" dataKey="organic" stroke="#3B82F6" strokeWidth={2.5} dot={false}/><Line type="monotone" dataKey="leads" stroke="#8B5CF6" strokeWidth={2.5} dot={false}/><Line type="monotone" dataKey="conversions" stroke="#10B981" strokeWidth={2.5} dot={false}/></LineChart></ResponsiveContainer></div></div>
        </div>
