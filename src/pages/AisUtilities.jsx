...
const performanceData = [
  { m: "Jan", organic: 900, leads: 8, conversions: 3 },
  { m: "Feb", organic: 1200, leads: 14, conversions: 6 },
  { m: "Mar", organic: 1800, leads: 22, conversions: 12 },
  { m: "Apr", organic: 2600, leads: 36, conversions: 24 },
  { m: "May", organic: 3400, leads: 54, conversions: 42 },
  { m: "Jun", organic: 4600, leads: 78, conversions: 66 },
];
    ...
    <div className="bg-white text-slate-900">
      <div className="max-w-7xl mx-auto px-5 lg:px-8 pt-6 pb-2">
        <nav className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <HiChevronRight className="w-3.5 h-3.5" />
          <span className="text-gray-900 font-semibold">AIS Utilities</span>
        </nav>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-5 lg:px-8 py-10 lg:py-14">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="text-4xl lg:text-5xl font-black leading-[1.08] text-gray-900 mb-3">AIS Utilities</h1>
            <h2 className="text-3xl lg:text-4xl font-black text-blue-700 leading-tight mb-5">180% Increase In Conversions</h2>
            <p className="text-gray-500 text-base leading-relaxed mb-6 max-w-lg">AIS Utilities engaged DeepRank to modernise their site and scale user acquisition for service bookings.</p>
            <div className="flex flex-wrap gap-2 mb-8">{["SEO","Local","Conversion","Paid"].map(t=>(<span key={t} className="text-xs font-semibold text-blue-700 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-full">{t}</span>))}</div>
          ...
          <div className="relative"><div className="rounded-3xl overflow-hidden shadow-2xl"><img src={aisutilities} alt="AIS Utilities" className="w-full h-80 lg:h-96 object-cover"/></div></div>
        </div>
      </section>

      <div className="bg-gray-50 border-y border-gray-100"><div className="max-w-7xl mx-auto px-5 lg:px-8 py-4"><div className="flex flex-wrap gap-6 lg:gap-12">{[{label:"Industry",value:"Utilities"},{label:"Location",value:"UK"},{label:"Duration",value:"7 Months"},{label:"Services Used",value:"Local SEO + CRO + Paid"}].map(i=>(<div key={i.label} className="flex items-center gap-3"><div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{i.label}</p><p className="text-sm font-bold text-gray-900 mt-0.5">{i.value}</p></div></div>))}</div></div></div>
      ...
      <div className="max-w-7xl mx-auto px-5 lg:px-8 py-12 lg:py-16 space-y-16">
        <div className="grid lg:grid-cols-3 gap-8"><div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3">About The Client</p><h3 className="text-xl font-black text-gray-900 mb-4">AIS Utilities</h3><p className="text-sm text-gray-600 leading-relaxed">AIS Utilities is a modern utilities provider looking to increase bookings via digital channels.</p></div><div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3">The Challenge</p><h3 className="text-xl font-black text-gray-900 mb-4">What We Had to Solve</h3><ul className="flex flex-col gap-2.5">{["Low local visibility","Fragmented listings","Poor booking flow","High CPA"].map(i=>(<li key={i} className="flex items-start gap-2 text-sm text-gray-600"><HiXCircle className="w-4 h-4 text-red-400 shrink-0 mt-0.5"/>{i}</li>))}</ul></div><div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3">Before Working With Us</p><h3 className="text-xl font-black text-gray-900 mb-4">Starting Point</h3><div className="grid grid-cols-2 gap-3">{[{v:"900",l:"Monthly Traffic"},{v:"18",l:"Bookings / Month"},{v:"£48",l:"AOV"},{v:"0.9%",l:"Conversion Rate"}].map(s=>(<div key={s.l} className="bg-red-50 border border-red-100 rounded-2xl p-4 text-center"><p className="text-2xl font-black text-red-600">{s.v}</p><p className="text-[10px] text-gray-500 font-medium mt-1">{s.l}</p></div>))}</div></div></div>

        <div><div className="mb-8"><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-2">Our Strategy</p><h3 className="text-3xl font-black text-gray-900">Local-first SEO and booking optimisation.</h3></div><div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">{[{n:"01",title:"Local SEO",desc:"GBP optimisation & citations"},{n:"02",title:"Listing Sync",desc:"Accurate NAP across directories"},{n:"03",title:"Booking UX",desc:"Reduce friction in booking flow"},{n:"04",title:"Paid Search",desc:"Target high-intent queries"},{n:"05",title:"Reporting",desc:"Bookings attribution"}].map(step=>(<div key={step.n} className="bg-gray-50 rounded-2xl p-5 border border-gray-100"><span className="text-4xl font-black text-blue-100">{step.n}</span><h4 className="text-sm font-black text-gray-900 mt-2 mb-2">{step.title}</h4><p className="text-xs text-gray-500 leading-relaxed">{step.desc}</p></div>))}</div></div>

        <div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-6">Services Used</p><div className="flex flex-wrap gap-3">{"Local SEO,CRO,Paid,Listings,Analytics".split(',').map(s=>(<span key={s} className="rounded-full border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm">{s}</span>))}</div></div>

        <div className="grid lg:grid-cols-2 gap-12 items-start"><div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3">What We Did</p><h3 className="text-3xl font-black text-gray-900 mb-6">End-to-end local growth program.</h3><div className="grid sm:grid-cols-2 gap-x-8 gap-y-2.5">{"GBP fixes,Listing sync,Landing pages,Booking UX,Paid ads,Automation,Reporting,Support".split(',').map(item=>(<div key={item} className="flex items-center gap-2 text-sm text-gray-700"><HiCheckCircle className="w-4 h-4 text-blue-600 shrink-0"/>{item}</div>))}</div></div><div className="bg-blue-900 rounded-3xl p-8 text-white"><p className="text-5xl text-blue-400 font-black leading-none mb-4">"</p><p className="text-base leading-relaxed text-blue-100 mb-6">"DeepRank helped us modernise our digital channels and scale bookings."</p><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-blue-700 flex items-center justify-center text-white font-black text-sm">AI</div><div><p className="text-sm font-bold">AIS Utilities</p><p className="text-xs text-blue-300">CEO</p></div></div></div></div>

        <div><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-2">The Results</p><h3 className="text-3xl font-black text-gray-900 mb-10">Improved bookings and lower CPA.</h3>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">{[{label:"Bookings",before:"18",after:"50",delta:"+177%",color:"#3B82F6",grad:"og",data:trafficData,positive:true},{label:"Traffic",before:"900",after:"4.6K",delta:"+411%",color:"#8B5CF6",grad:"le",data:trafficData,positive:true},{label:"AOV",before:"£48",after:"£62",delta:"+29%",color:"#10B981",grad:"cp",data:leadsData,positive:true},{label:"Conversion",before:"0.9%",after:"2.3%",delta:"+156%",color:"#F59E0B",grad:"ic",data:leadsData,positive:true}].map(m=>(<div key={m.label} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-3">{m.label}</p><div className="flex items-end justify-between mb-1"><p className="text-3xl font-black text-gray-900">{m.after}</p><span className={`text-xs font-bold px-2 py-0.5 rounded-lg ${m.positive?"bg-green-50 text-green-700":"bg-red-50 text-red-700"}`}>{m.delta}</span></div><p className="text-[10px] text-gray-400 mb-3">was {m.before}</p><MetricChart data={m.data} color={m.color} gradient={m.grad} /></div>))}</div>
