...
import { HiBolt, HiCheckCircle, HiLink, HiArrowTrendingUp } from "react-icons/hi2";
    ...
    visual: (
      <div className="flex items-center justify-center h-full">
        <div className="relative">
          <div className="w-10 h-10 rounded-full bg-blue-400/30 border border-blue-300/40 flex items-center justify-center text-lg">🤖</div>
      ...
      <div className="flex flex-col justify-end h-full px-2 pb-1">
        <div className="flex items-end gap-0.5 mb-1 h-10">
          {[30, 50, 40, 70, 55, 80, 65, 90].map((h, i) => (
            <div key={i} className="flex-1 rounded-sm bg-green-400 opacity-80" style={{ height: `${h}%` }} />
          ))}
        </div>
  ...
  },
  {
    ...
    visual: (
      <div className="flex flex-col gap-1 items-center justify-center h-full">
        {[{ l: "f", c: "bg-blue-500" }, { l: "in", c: "bg-blue-600" }, { l: "G", c: "bg-red-500" }].map((s) => (
          <div key={s.l} className={`w-6 h-6 ${s.c} rounded-md flex items-center justify-center text-white text-[10px] font-bold`}>{s.l}</div>
        ))}
      </div>
    ),
  },
    ...
    visual: (
      <div className="flex flex-col justify-center h-full px-2 gap-0.5">
        {["const app = () => {", "  return <UI />;", "}"].map((l, i) => (
          <div key={i} className="text-[8px] font-mono text-green-400 opacity-80">{l}</div>
        ))}
      </div>
    ),
  },
      ...
      <div className="flex items-center justify-center h-full">
        <div className="border border-blue-300/30 rounded-lg p-1.5 bg-blue-900/50">
          <div className="bg-blue-300/20 rounded w-12 h-7 flex items-center justify-center">
            <span className="text-[7px] text-white font-semibold">DeepRank</span>
          </div>
    ...
    visual: (
      <div className="flex items-center justify-center h-full">
        <div className="relative w-10 h-11">
          <div className="w-full h-full bg-purple-400/40 border border-purple-300/50" style={{ clipPath: "polygon(50% 0%,100% 25%,100% 75%,50% 100%,0% 75%,0% 25%)" }} />
          <div className="absolute inset-0 flex items-center justify-center text-purple-200 text-[9px] font-bold">NFT</div>
        </div>
      </div>
    ),
  },
  {
    ...
    visual: (
      <div className="grid grid-cols-2 gap-1 p-2 h-full items-center">
        {["⚙️", "🔗", "📊", "🛠️"].map((e, i) => (
          <div key={i} className="w-6 h-6 bg-indigo-400/30 rounded-md flex items-center justify-center text-xs border border-indigo-300/30">{e}</div>
        ))}
      </div>
    ),
  },
  ...
  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[860px] max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden mega-menu-animate">
    <div className="grid grid-cols-[200px_1fr_220px]">
      ...
      <div className="p-4 border-r border-gray-100">
        <p className="text-[10px] font-bold tracking-widest text-gray-400 uppercase px-2 mb-3">Our Products</p>
        {products.map((p) => (
          <div key={p.name} className="flex items-start gap-3 p-2.5 rounded-xl hover:bg-blue-50/60 cursor-pointer group transition-colors">
            <div className={`w-8 h-8 shrink-0 rounded-lg flex items-center justify-center font-bold text-sm ${p.iconBg} ${p.iconColor}`}>{p.icon}</div>
            <div>
              <p className="font-semibold text-gray-900 text-sm leading-tight">{p.name}</p>
              <p className="text-[11px] text-gray-400 leading-snug mt-0.5">{p.desc}</p>
            </div>
          </div>
        ))}
      ...
      <div className="p-5">
        <p className="text-[10px] font-bold tracking-widest text-blue-600 uppercase mb-2">Product Overview</p>
        <h3 className="text-lg font-bold text-gray-900 leading-snug mb-2">
          Powerful Products.<br />Built for Businesses.
        </h3>
        <p className="text-xs text-gray-500 leading-relaxed mb-4">
          Discover our suite of products designed to automate, manage, and grow your business.
        </p>
        {/* Mini dashboard */}
        <div className="rounded-xl border border-gray-200 bg-gray-50 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-900 to-indigo-900 text-white px-3 py-1.5 flex items-center justify-between">
            <div className="flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-red-400" /><div className="w-1.5 h-1.5 rounded-full bg-yellow-400" /><div className="w-1.5 h-1.5 rounded-full bg-green-400" /></div>
            <span className="text-[9px] font-medium opacity-80">DeepRank Dashboard</span>
      ...
      <div className="p-5 bg-gray-50/80 border-l border-gray-100">
        <p className="text-[10px] font-bold tracking-widest text-blue-600 uppercase mb-2">Why Choose Us?</p>
        <h3 className="text-sm font-bold text-gray-900 leading-snug mb-4">All-in-One Solutions to Scale</h3>
        <div className="flex flex-col gap-3 mb-4">
          {whyChoose.map((item) => (
            <div key={item.title} className="flex items-start gap-2.5">
              <div className="w-6 h-6 shrink-0 rounded-lg bg-blue-100 flex items-center justify-center mt-0.5">{item.icon}</div>
              <div>
        ...
        <div className="bg-blue-900 rounded-xl p-3 text-white">
          ...
          <p className="text-[11px] text-blue-300 mb-2.5">Our experts are here to help you find the right product.</p>
...
const ServiceMegaMenu = () => (
  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[980px] max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden max-h-[82vh] overflow-y-auto mega-menu-animate">
    ...
    <div className="grid grid-cols-[1fr_220px]">
      <div className="p-5">
        <p className="text-[10px] font-bold tracking-widest text-gray-400 uppercase mb-3">Our Services</p>
        <div className="grid grid-cols-4 gap-2.5 mb-2.5">
          {serviceCards.slice(0, 4).map((s) => (
            <div key={s.name} className={`rounded-xl bg-gradient-to-br ${s.gradient} h-24 cursor-pointer group overflow-hidden relative hover:scale-[1.03] transition-transform duration-200 shadow-sm`}>
      ...
      <div className="bg-gray-50 border-l border-gray-100 p-4 flex flex-col">
        <p className="text-[10px] font-bold tracking-widest text-blue-600 uppercase mb-2">Featured Service</p>
        <h3 className="text-base font-bold text-gray-900 leading-snug mb-1.5">AI Automation</h3>
        <p className="text-[11px] text-gray-500 leading-relaxed mb-3">
          Build AI employees that work 24/7, automate workflows, qualify leads and transform efficiency.
        </p>
        <button className="self-start flex items-center gap-1 bg-blue-900 text-white text-[11px] font-semibold px-3 py-1.5 rounded-lg hover:bg-blue-800 transition mb-3">
    ...
    <div className="border-t border-gray-100 bg-gray-50/60 px-5 py-4 grid grid-cols-[1fr_1fr_1fr_1fr_180px] gap-5">
      {serviceCategories.map((cat) => (
        <div key={cat.label}>
          <p className={`text-[10px] font-bold tracking-widest uppercase mb-2.5 ${cat.color}`}>{cat.label}</p>
          <ul className="flex flex-col gap-1.5">
            {cat.items.map((item) => (
              <li key={item} className="text-[11px] text-gray-500 hover:text-blue-700 cursor-pointer transition-colors">{item}</li>
            ))}
          </ul>
        </div>
      ))}
      <div className="bg-white rounded-xl border border-gray-200 p-3 shadow-sm flex flex-col justify-between">
        <div>
          ...
          <p className="text-[11px] text-gray-400 leading-snug">Book a free discovery call with our experts.</p>
        </div>
...
const Navbar = () => {
  ...
  const openMenu = (name) => { clearTimeout(hideTimeout.current); setActiveMenu(name); };
  const closeMenu = () => { hideTimeout.current = setTimeout(() => setActiveMenu(null), 150); };
      ...
      <header className="relative w-full bg-white/95 backdrop-blur-sm border-b border-gray-100/80 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-5 lg:px-8 h-[72px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-xl lg:text-2xl font-bold tracking-tight text-gray-900">DeepRank</h1>
          </div>
          ...
          <nav className="hidden lg:flex">
            <ul className="flex items-center gap-8 text-gray-600 text-sm font-medium">
              <li className="cursor-pointer hover:text-blue-700 transition-colors"
                onMouseEnter={() => openMenu("products")} onMouseLeave={closeMenu}>
                <span className={`flex items-center gap-1 py-1 ${activeMenu === "products" ? "text-blue-700" : ""}`}
              ...
              <li className="cursor-pointer hover:text-blue-700 transition-colors"
                onMouseEnter={() => openMenu("services")} onMouseLeave={closeMenu}>
                <span className={`flex items-center gap-1 py-1 ${activeMenu === "services" ? "text-blue-700" : ""}`}
              ...
              <li className="cursor-pointer hover:text-blue-700 transition-colors py-1">Contact Us</li>
            </ul>
          </nav>
          ...
          <div className="lg:hidden absolute top-full left-0 w-full bg-white shadow-2xl border-t border-gray-100 z-50">
            <div className="px-5 py-5">
              <ul className="flex flex-col gap-1 text-gray-700 font-medium text-sm mb-5">
                {["Products", "Services", "Case Study", "Contact Us"].map((item) => (
                  <li key={item} className="py-3 px-4 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer">{item}</li>
                ))}
              </ul>
