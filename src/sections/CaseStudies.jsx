  ...
  <section className="py-14 lg:py-20 bg-gray-50">
    <div className="max-w-7xl mx-auto px-5 lg:px-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
        ...
        <Link
          to="/case-study"
          className="self-start sm:self-auto flex items-center gap-2 text-sm font-semibold text-blue-700 border border-blue-200 bg-blue-50 hover:bg-blue-100 px-4 py-2.5 rounded-xl transition-colors whitespace-nowrap"
        >
          View all Case Studies <HiArrowRight className="w-4 h-4" />
        </Link>
      </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {cases.map((c) => (
          <div
            key={c.title}
            className="rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer bg-white"
          >
            <div className="relative h-52 overflow-hidden">
              ...
              <span className={`absolute top-4 left-4 text-[10px] font-bold px-2.5 py-1 rounded-full ${c.tagBg}`}>
                {c.tag}
              </span>
              <Link
                to={c.to}
              ...
              <div className="absolute bottom-4 left-4">
                <div className="bg-white rounded-xl px-4 py-2 shadow-lg inline-block">
                  <p className="text-2xl font-black text-[#08104A] leading-none">{c.stat}</p>
                  <p className="text-[10px] text-gray-500 font-medium mt-0.5">{c.statLabel}</p>
                </div>
              </div>
            </div>
            <div className="p-5">
              <h3 className="text-sm font-bold text-gray-900 leading-snug mb-2">{c.title}</h3>
              <p className="text-gray-500 text-xs leading-relaxed mb-4">{c.desc}</p>
              <Link
                to={c.to}
                className="flex items-center gap-1.5 text-blue-700 font-semibold text-xs hover:gap-3 transition-all"
              >
